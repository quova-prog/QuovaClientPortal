...
test('WorkOS login and signup pages redirect through AuthKit instead of rendering password forms', () => {
  ...
  assert.match(signup, /signUp\('', '', '', '', workosSignupInviteToken \?\? null\)/s)
...
test('WorkOS invite tokens are detected separately from legacy Supabase UUID invites', () => {
  ...
  assert.match(signup, /const workosSignupInviteToken = inviteToken \?\? readRememberedWorkosInviteToken\(\)/s)
  assert.match(signup, /else if \(!workosSignupInviteToken\) clearRememberedWorkosInviteToken\(\)/s)
  assert.match(signup, /signUp\('', '', '', '', workosSignupInviteToken \?\? null\)/s)
