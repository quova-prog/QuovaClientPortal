...
test('WorkOS login and signup pages redirect through AuthKit instead of rendering password forms', () => {
  ...
  assert.match(login, /void signIn\([^)]*workosInviteToken/s)
  ...
  assert.match(signup, /signIn\('', '', workosInviteToken \?\? null\)/s)
...
test('WorkOS invite tokens are detected separately from legacy Supabase UUID invites', () => {
  ...
  assert.match(signup, /const workosInviteToken = inviteToken \?\? readRememberedWorkosInviteToken\(\)/s)
  assert.doesNotMatch(signup, /clearRememberedWorkosInviteToken\(\)/s)
  assert.match(signup, /signIn\('', '', workosInviteToken \?\? null\)/s)
  ...
  assert.match(login, /const workosInviteToken = inviteToken \?\? readRememberedWorkosInviteToken\(\)/s)
  assert.doesNotMatch(login, /clearRememberedWorkosInviteToken\(\)/s)
  assert.match(auth, /clearRememberedWorkosInviteToken\(\)/s)
  assert.match(auth, /await authKitSignIn\(options\)/s)
})
