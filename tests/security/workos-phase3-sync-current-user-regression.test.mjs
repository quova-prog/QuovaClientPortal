...
test('sync-current-user inserts or refreshes the local profile from WorkOS claims', () => {
  ...
  assert.match(fn, /getWorkosUser/s)
  assert.match(fn, /resolveWorkosFullName\(identity\)/s)
  assert.match(fn, /workosUserFullName\(user\)/s)
  assert.match(fn, /select\('id, org_id, role, email, full_name, membership_status, deactivated_at'\)/s)
  ...
  assert.match(fn, /full_name:\s*workosFullName/s)
...
test('sync-current-user does not update an unchanged existing profile during login bootstrap', () => {
  ...
  assert.match(fn, /const existingFullName = cleanProfileName\(existingProfile\.full_name\)/s)
  assert.match(fn, /const shouldBackfillFullName = !existingFullName && !!workosFullName/s)
  assert.match(fn, /existingProfile\.role === identity\.role && existingProfile\.email === nextEmail && !shouldBackfillFullName/s)
